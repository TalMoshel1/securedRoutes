...
const Days = () => {
  ...
  const triggerRefetch = useSelector(
    (state) => state.calendar.triggerRefetch
  );
  ...
  useEffect(()=>{
    console.log('fourWeeksCounter: ',fourWeeksCounter.count)
  },[fourWeeksCounter])
    ...
    const sendPostRequest = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:3000/api/lessons/month",
          {
            method: "POST",
            headers: {
